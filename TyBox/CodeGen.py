...
def gen_network_struct(layer_sizes_list):
    ...
    \n\
        uint8_t mbp_buffer[BUFFER_SIZE][FEATURE_SIZE + 1]; //last number in the list is the label\n\
        int buf_index;\n\
        bool is_full;\n\
    \n\
    \n\
...
def gen_train():
    res = "void train(t_dense_network & d_network, float lr, float output_scale, float output_zero_point){\n\
    \n\
    ...
    \n\
          ...
          float dequantized_buffer_element;\n\
          for(int j=0; j<FEATURE_SIZE; j++){\n\
            // dequantize d_network.mbp_buffer[i][j]\n\
            dequantized_buffer_element = (float(d_network.mbp_buffer[i][j]) - output_zero_point) * output_scale;\n\
            d_network.activations_list[0][j] = dequantized_buffer_element;\n\
          }\n\
          //train\n\
...
def gen_push():
    res = "void push_datum_into_buf(t_dense_network & d_network, uint8_t * output, int label){//TfLiteTensor* output, int label){\n\
    \n\
...
def gen_evaluate():
    res = "bool evaluate(t_dense_network & d_network, uint8_t * output, int label, float output_scale, float output_zero_point){// TfLiteTensor* output, int label){\n\
        //load into the network\n\
        float fe_dequantized_output;\n\
        for(int input_node_index=0; input_node_index<FEATURE_SIZE; input_node_index++){\n\
            ...
            fe_dequantized_output = (float(output[input_node_index]) - output_zero_point) * output_scale;\n\
            //d_network.activations_list[0][input_node_index] = float(output->data.f[input_node_index]);\n\
            d_network.activations_list[0][input_node_index] = float(fe_dequantized_output);\n\
        }\n\
        //calculate output\n\
        forward_pass(d_network);\n\
